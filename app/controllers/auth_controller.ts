...
export default class AuthController {
  async register ({request, response}: HttpContext){
    ...
    return response.created({ message: `Bienvenue, ${payload.firstname} ! Votre compte a été créé avec succès.` });
  }
  ...
  async login({auth, request, response}: HttpContext){
    ...
    try {
      ...
      return response.status(500).json({ message: "Une erreur s'est produite lors de la tentative de connexion. Veuillez réessayer plus tard." })
    }
  }
}
